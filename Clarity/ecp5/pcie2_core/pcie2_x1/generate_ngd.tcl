...
file mkdir syn_results
	...
	if [catch {open $Para(ModuleName).cmd w} rspFile] {
				...
				puts $rspFile "verilog_file_list: \"../pcie_eval/models/$Para(tech)/$Para(ModuleName)_sci_ctrl.v\""
				...
				puts $rspFile "frequency: 250.000"
	...
	if [runCmd "\"$synpwrap_cmd\" -rem -e $Para(ModuleName) -target ecp5um5g"] {
				return
	} else {
				vwait done
				if [checkResult $done] {
